...
int bamr_class::compute_isrc(size_t ix, const ubvector &pars, 
                            double &log_wgt, model_data &dat) {
  model &m=*this->mod;
  ns_pop &nsp = nsd->pop;
  size_t np_eos=m.n_eos_params;
  ...
  if (rad<nsd->source_tables[ix].get_x_data()[0] ||
      rad>nsd->source_tables[ix].get_x_data()
      [nsd->source_tables[ix].get_nx()-1] ||
      mass<nsd->source_tables[ix].get_y_data()[0] ||
      mass>nsd->source_tables[ix].get_y_data()
      [nsd->source_tables[ix].get_ny()-1]) {
    wgt_star=0.0;
  } else {
    if (nsd->source_fnames_alt.size()>0) {
      //if (d_atm<2.0/3.0) {
  ...
  if (wgt_star<=0.0) {
    wgt_star=o2scl::matrix_max_value<ubmatrix,double>
             (nsd->source_tables[ix].get_slice(name))/1.0e8;
  }
    
  if (nsd->source_names[ix]!=string("0030")) {
    double sn_mass=nsp.skewed_norm(mass,mean,width,skew);
    log_wgt=log(wgt_star)+log(sn_mass);
  } else {
...
int bamr_class::compute_dist(size_t ix, const ubvector &pars, 
  ...
  double M_max=dat.m_max, fsn, mass;
  double wgt_ns=1.0, wgt_wd=1.0, wgt_lx=1.0;

  for(size_t i=0; i<2; i++) {
    mass=mass_gw17[i];
    fsn=nsp.skewed_norm(mass, mean_ns, width_ns, skew_ns);
    wgt_ns*=fsn;
  }
  for(size_t i=0; i<2; i++) {
    mass=mass_gw19[i];
  ...
  for(size_t i=0; i<pd.n_dns; i++) {
    mass=pars[pvi["M_"+pd.id_ns[i]]];
    fsn=nsp.skewed_norm(mass, mean_ns, width_ns, skew_ns);
    wgt_ns*=fsn;
  }
  for(size_t i=0; i<pd.n_nswd; i++) {
    mass=pars[pvi["M_"+pd.id_wd[i]]];
    fsn=nsp.skewed_norm(mass, mean_wd, width_wd, skew_wd);
    wgt_wd*=fsn;
  }
  for(size_t i=0; i<pd.n_lmxb; i++) {
    mass=pars[pvi["M_"+pd.id_lx[i]]];
    fsn=nsp.skewed_norm(mass, mean_lx, width_lx, skew_lx);
    wgt_lx*=fsn;
  }
  for(size_t i=0; i<nsd->n_sources; i++) {
    if (nsd->source_names[i]!=string("0030")) {
      mass=M_max*pars[pvi["mf_"+nsd->source_names[i]]];
      fsn=nsp.skewed_norm(mass, mean_lx, width_lx, skew_lx);
      wgt_lx*=fsn;
    }
  }
...
int bamr_class::numeric_deriv(size_t ix, ubvector &x, point_funct &pf,
  ...
  // Adjust step size
  double epsrel, epsmin=1.0e-15;
  
  if (ix==0)  epsrel=1.0e-4;
  if (ix==1)  epsrel=1.0e-4;
  if (ix==2)  epsrel=1.0e-3;
  if (ix==3)  epsrel=1.0e-3;
  if (ix==4)  epsrel=1.0e-3;
  if (ix==5)  epsrel=1.0e-3;
  if (ix==6)  epsrel=1.0e-3;
  if (ix==7)  epsrel=1.0e-3;
  if (ix==8)  epsrel=1.0e-3;
  if (ix==9)  epsrel=1.0e-5;
  if (ix==10) epsrel=1.0e-4;
  if (ix==11) epsrel=1.0e-3;
  if (ix==12) epsrel=1.0e-3;
  if (ix==13) epsrel=1.0e-3;
  if (ix==14) epsrel=1.0e-3;
  if (ix==15) epsrel=1.0e-3;
  if (ix==16) epsrel=1.0e-3;
  if (ix==17) epsrel=1.0e-3;
  if (ix==18) epsrel=1.0e-3;
  if (ix==19) epsrel=1.0e-3;
  if (ix==20) epsrel=1.0e-3;
  if (ix==21) epsrel=1.0e-3;
  if (ix==22) epsrel=1.0e-3;
  if (ix==23) epsrel=1.0e-3;
  if (ix==24) epsrel=1.0e-3;
  if (ix==25) epsrel=1.0e-3;
  if (ix==26) epsrel=1.0e-3;
  if (ix==27) epsrel=1.0e-3;
  if (ix==28) epsrel=1.0e-3;
  if (ix==29) epsrel=1.0e-3;
  if (ix==30) epsrel=1.0e-3;
  if (ix==31) epsrel=1.0e-3;
  if (ix==32) epsrel=1.0e-3;
  if (ix==33) epsrel=1.0e-3;
  if (ix==34) epsrel=1.0e-3;
  if (ix==35) epsrel=1.0e-3;
  if (ix==36) epsrel=1.0e-3;
  if (ix==37) epsrel=1.0e-3;
  if (ix==38) epsrel=1.0e-4;
  if (ix==39) epsrel=1.0e-3;
  if (ix==40) epsrel=1.0e-5;
  if (ix==41) epsrel=1.0e-5;
  if (ix==42) epsrel=1.0e-3;
  if (ix==43) epsrel=1.0e-3;
  if (ix==44) epsrel=1.0e-4;
  if (ix==45) epsrel=1.0e-4;
  if (ix==46) epsrel=1.0e-3;
  if (ix==47) epsrel=1.0e-3;
  if (ix==48) epsrel=1.0e-3;
  if (ix==49) epsrel=1.0e-3;
  if (ix==50) epsrel=1.0e-3;
  if (ix==51) epsrel=1.0e-3;
  if (ix==52) epsrel=1.0e-3;
  if (ix==53) epsrel=1.0e-3;
  if (ix==54) epsrel=1.0e-3;
  if (ix==55) epsrel=1.0e-3;
  if (ix==56) epsrel=1.0e-3;
  if (ix==57) epsrel=1.0e-3;
  if (ix==58) epsrel=1.0e-3;
  if (ix==59) epsrel=1.0e-3;
  if (ix==60) epsrel=1.0e-3;
  if (ix==61) epsrel=1.0e-3;
  if (ix==62) epsrel=1.0e-3;
  if (ix==63) epsrel=1.0e-3;
  if (ix==64) epsrel=1.0e-3;
  if (ix==65) epsrel=1.0e-3;
  if (ix==66) epsrel=1.0e-3;
  if (ix==67) epsrel=1.0e-3;
  if (ix==68) epsrel=1.0e-3;
  if (ix==69) epsrel=1.0e-3;
  if (ix==70) epsrel=1.0e-3;
  if (ix==71) epsrel=1.0e-3;
  if (ix==72) epsrel=1.0e-3;
  if (ix==73) epsrel=1.0e-3;
  if (ix==74) epsrel=1.0e-3;
  if (ix==75) epsrel=1.0e-3;
  if (ix==76) epsrel=1.0e-3;
  if (ix==77) epsrel=1.0e-3;
  if (ix==78) epsrel=1.0e-3;
  if (ix==79) epsrel=1.0e-3;
  if (ix==80) epsrel=1.0e-3;
  if (ix==81) epsrel=1.0e-3;
  if (ix==82) epsrel=1.0e-3;
  if (ix==83) epsrel=1.0e-3;
  if (ix==84) epsrel=1.0e-3;
  if (ix==85) epsrel=1.0e-3;
  if (ix==86) epsrel=1.0e-3;
  if (ix==87) epsrel=1.0e-3;
  if (ix==88) epsrel=1.0e-3;
  if (ix==89) epsrel=1.0e-3;
  if (ix==90) epsrel=1.0e-3;
  if (ix==91) epsrel=1.0e-3;
  if (ix==92) epsrel=1.0e-3;
...
int bamr_class::compute_deriv(ubvector &pars, point_funct &pf, 
  ...
  for(size_t j=0; j<pd.n_dns; j++) {
    ...
    sn_nsp.push_back(nsp.sn_ns[j]);
    an_nsp.push_back(nsp.an_ns[j]);
  }
  for(size_t j=0; j<pd.n_nswd; j++) {
    c_fsn_wd*=nsp.sn_wd[j];
    sn_nsp.push_back(nsp.sn_wd[j]);
    an_nsp.push_back(nsp.an_wd[j]);
  }
  for(size_t j=0; j<pd.n_lmxb; j++) {
    c_fsn_lx*=nsp.sn_lx[j];
    sn_nsp.push_back(nsp.sn_lx[j]);
    an_nsp.push_back(nsp.an_lx[j]);
  }
  ...
  while (ip<np) {
    ...
    if (ip==np_eos) { // w.r.t. M_chirp_det, q, z_cdf
      ...
      for (size_t j=0; j<3; j++) {
        int ret=numeric_deriv(ip, pars, pf_gw17, pfx1, gfx, dat);
        if (ret!=o2scl::success) {
          cout << "bamr_class::compute_deriv() failure." << endl;
          return m.ix_grad_failed;
        }
    ...
    if (ip==np_eos+np_ligo-1) { // w.r.t. m1_gw19
      pfx1=log(wgt_gw19*fsn_gw19[0]*fsn_gw19[1]);
      int ret=numeric_deriv(ip, pars, pf_gw19, pfx1, gfx, dat);
      if (ret!=o2scl::success) {
        cout << "bamr_class::compute_deriv() failure." << endl;
        return m.ix_grad_failed;
      }
    ...
    if (ip==np_eos+np_ligo) { // w.r.t. mf_*
      for(size_t j=0; j<np_src; j++) {
        point_funct pf_src;
        pf_src=bind(mem_fn<int(size_t,const ubvector&,double&,model_data&)>
                         (&bamr_class::compute_isrc),this,ref(j),_2,_3,_4);
        pfx1=log(wgt_em[j]*fsn_em[j]);
        int ret=numeric_deriv(ip, pars, pf_src, pfx1, gfx, dat);
        if (ret!=o2scl::success) {
          cout << "bamr_class::compute_deriv() failure." << endl;
          return m.ix_grad_failed;
        }
    ...
    if (ip==np_eos+np_ligo+np_src) { // w.r.t. mean_*, width_*, skew_*
      for (size_t j=0; j<np_dist; j++) {
        ...
        int ret=numeric_deriv(ip, pars, pf_dist, pfx1, gfx, dat);
        if (ret!=o2scl::success) {
          cout << "bamr_class::compute_deriv() failure." << endl;
          return m.ix_grad_failed;
        }
    ...
    if (ip==np_eos+np_ligo+np_src+np_dist) { // w.r.t. M_*
      for (size_t j=0; j<np_nsp; j++) {
        ...
        pfx1=log(an_nsp[j]*sn_nsp[j]);
        cout << "j=" << j+34 << ", an=" << an_nsp[j] << ", sn=" << sn_nsp[j] << endl;
        int ret=numeric_deriv(ip, pars, pf_mass, pfx1, gfx, dat);
        if (ret!=o2scl::success) {
          cout << "bamr_class::compute_deriv() failure." << endl;
          return m.ix_grad_failed;
        }
