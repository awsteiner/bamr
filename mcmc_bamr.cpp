...
int mcmc_bamr::mcmc_func(std::vector<std::string> &sv, bool itive_com) {
 ...
 if (set->apply_emu) {    
  ...
  if (mcmc_method==string("hmc")) {
    ...
    hmc_stepper->hmc_step[ 2]=1.0e-2*(high[2]-low[2]);
    hmc_stepper->hmc_step[ 3]=1.0e-2*(high[3]-low[3]);
    hmc_stepper->hmc_step[ 4]=1.0e-2*(high[4]-low[4]);
    hmc_stepper->hmc_step[ 5]=1.0e-3*(high[5]-low[5]);
    hmc_stepper->hmc_step[ 6]=1.0e-2*(high[6]-low[6]);
    hmc_stepper->hmc_step[ 7]=1.0e-3*(high[7]-low[7]);
    ...
    hmc_stepper->hmc_step[61]=1.0e-2*(high[61]-low[61]);
    hmc_stepper->hmc_step[62]=1.0e-2*(high[62]-low[62]);
    hmc_stepper->hmc_step[63]=1.0e-2*(high[63]-low[63]);
